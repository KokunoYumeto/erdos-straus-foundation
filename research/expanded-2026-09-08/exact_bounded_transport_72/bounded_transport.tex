...
This text retains the full prime-shell system in the supplied notes
\emph{Untitled 1736} and \emph{Untitled 1737}.  Its new cross-shell
calculations classify ratio-preserving transport and the two elementary
positive unimodular shears.  The local gluing calculation keeps the
finite exponent bounds through its integral lifts.  The connection to
Alain Connes and Caterina Consani's divisor-category construction uses
the embedded ordered groups and their exact reciprocal-divisor locus.
Every arithmetic assertion used here is proved below; the accompanying
exact computations check specified finite instances.  Neither a universal
occupancy theorem nor a global empty prime is established.
...
\input{first_two_shell_sieve.tex}
\input{counterexample_sieve/all_shell_no_hit.tex}
...
\input{shared_variable_crt.tex}
\input{raw_scale_hit_incidence.tex}
...
\input{exact_audit/fixed_y_successor.tex}
...
\input{ns_operator_bridge/sieve_character_cover.tex}
\input{ns_operator_bridge/reverse_proof_transport.tex}
\input{ns_operator_bridge/reverse_smooth_inverse.tex}
