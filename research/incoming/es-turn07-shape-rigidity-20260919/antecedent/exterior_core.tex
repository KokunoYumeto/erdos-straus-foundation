...
The Type I/II parametrizations are classical \cite[\S2, Propositions 2.2 and
2.6]{ET}. Their algebraic identities are proved below in the retained marking.
No historical-priority claim is made for the derived cutoff or atlas.

\section{The original exterior state and its three arithmetic parameters}
Let $p>1$, $p\equiv1\pmod4$, and retain the unit first-half source
\[
 p/4<a<p/2,\qquad \gcd(p,a)=1,\qquad R=4a-p,
 \qquad u\mid a^2,\quad R\mid4u+1.                         \tag{1}
\]
...
$R\mid4r^2+1$, which is impossible. Hence $h>1$. Further,
...
The function $2(w+1)\sqrt{w-1}-w$ is strictly increasing for $w>1$.
At $w=1$, (8) holds, and at $w=p$ it fails since
$(p+1)^2(p-1)-p^2=p^3-p-1>0$. There is therefore one final true integer in
$[1,p-1]$, obtained by binary search. Expanding the increasing expression gives
$2w^{3/2}-w+O(\sqrt w)$, whence (9).
\end{proof}
...
\[
 ...
 \quad w\mid B_D^2,\quad w<B_D,\quad D\mid w+B_D.
\]
Set
\[
 g=\gcd(B_D,w),\quad
...
\end{theorem}
\begin{proof}
The direct chart is literal. For the reciprocal chart, gcd normalization gives
$4hr\kappa=pD+1$, $r<\kappa$, $\gcd(r,\kappa)=1$, and
$D\mid r+\kappa$. The number $D$ is coprime to $hr\kappa$, so
$\gcd(r,s)=1$. Also
\[
 R\kappa=pr+s>0,
\]
...
\[
 ...
 =2hr(\kappa-r)-1>0.
\]
Equivalently, $2a<p$; the same identity follows by substituting
$s=(r+\kappa)/D$. Thus $R>0$, $a<p/2$, and this is the original exterior state.
The inverse recovers $B_D=hr\kappa$ and the orientation $w=hr^2<B_D$.
The complementary reciprocal divisor would give $\kappa<r$ and is not silently
identified with this orientation.

For (12), $v<p$ implies $\gcd(v,p)=1$. Since $R\mid p^2+4v$, we have
$\gcd(R,4v)=1$. The residue modulo $4K(v)$ gives the actual integer availability
$v\mid a^2$, so (13) defines $u$. Finally
\[
 4v(4u+1)=(p+R)^2+4v\equiv0\pmod R.
\]
Cancelling the proved unit $4v$ gives the exterior gate. The range gives the
first-half source, and gcd normalization applies. Conversely every original
...
The grade $v$ cannot tie either of the other parameters. A possible $R=D$ tie is
assigned to the direct chart. These rules are disjoint and exhaustive.
\end{proof}
...
parameters $p=97,929$ are prime. At $n=24+210j$, $j\ge0$, they are in the hard
residue class $289\pmod{840}$. No infinitude of prime values of the cubic is
asserted; consequently the asymptotic sharpness claim is for the stated unit
integer domain, not for a proved infinite prime subfamily.

The least complement grade is not automatically occupied. At $p=2521$, $v=11$
requires $R\mid6355485$ and $R\equiv31\pmod{44}$, $0<R<p$.
The factorization $6355485=3^2\cdot5\cdot141233$ gives the possible small divisors
$1,3,5,9,15,45$, none in that class. This antecedent extremal failure remains
an explicit negative control \cite{Localization}.
