...
The new conclusions are coordinate-level:
...
3. At radius8 the only possible nonsingular profile is (8,-4,11), and its grade
   ...
   required to obtain that positive return.
...
core.tex is the complete proof. workbench.tex compiles it. preprint.tex is a
standalone shorter account. MORPHISMS.md retains maps, fibres and exceptions.
...
at primes p=1 mod8 through the specified bound. The separate checker imports no
main or predecessor module and uses primitive h,r,s enumeration. The 52-profile
rigidity proof is complete; the separate s<=200000 quartic prefix is not claimed
complete at larger s. Read RELEASE_EXECUTION.json for actual command outcomes.
...
independent mathematical review, Lean build, or new overall ES verification
