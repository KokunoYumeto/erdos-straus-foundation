...
3. Shape map: alpha=v-R, beta=v-D, X=v,Y=2a. Inverse R=X-alpha,D=X-beta,
   ...
   explicitly specified original image. The cubic equation alone is insufficient.
...
6. The complete collision-map fibre retains odd t with R=2delta*t^2+1<p and
...
10. Radius-eight nonsingular E -> h=11,alpha=8,beta=-4 -> p=8 mod11 -> the
    ...
    Forgetting them is a many-to-one map, with at most three source states at p.
11. Rational cubic involution (X,Y)->(B/X,BY/X^2), B!=0,X!=0. It preserves the
    ...
    s1 changes residuals to1 mod4 and the reconstructed p to3 mod4.
    The surviving correspondence is explicitly recorded, not called unrelated.
