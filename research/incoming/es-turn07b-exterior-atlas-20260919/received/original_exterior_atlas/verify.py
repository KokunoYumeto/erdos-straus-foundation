...
def examples():
    ...
    fam=[]
    for n in [24+210*k for k in range(20)]:
        h=4*n*n-2;t0=4*n*n-1;p=4*h*n-t0
        z=state(p,h*n,h*n*n)
...
def main():
    ...
    start=time.monotonic();rows=[];total_vectors=0;total_states=0;chart_counts=Counter();candidate_counts=Counter()
    ...
    report=dict(success=True,bound=args.bound,primes=len(rows),original_vectors=total_vectors,
                ...
                check_types=dict(CHECKS),elapsed_seconds=time.monotonic()-start,
