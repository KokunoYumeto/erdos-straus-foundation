...
\boxed{
\begin{aligned}
&\text{On the separated residue stratum:}\\[2pt]
&\text{one denominator divisible by }p:\quad
\widetilde{\mathscr E}_p/\mathscr E_p
\cong(\mathbb Z/p\mathbb Z)^2,\\[4pt]
&\text{two denominators divisible by }p:\quad
\widetilde{\mathscr E}_p/\mathscr E_p
\cong
...
### The \(p\mid S\) case has an integral translated chart, related by an exact quadratic twist

When \(p\mid S\), the literal normalized coefficients are not all integral. Put

$$
F_0(T)=-\frac1S\prod_i(T-t_i).
$$

Take the recorded shift \(\lambda=1\) and define
...
$$
c_\lambda=\frac S{S-4\lambda}.
$$

The translated signed algebra is the quadratic twist of the original signed algebra by the square class of \(c_\lambda\).  More exactly, after adjoining an element \(q\) with
...
\qquad
\sigma_\lambda\longmapsto q\sigma_0
}
$$

is an isomorphism of signed algebras over \(\mathbb Q_p(q)\): translation carries the root polynomial to \(F_0(T_0)\), and

$$
(q\sigma_0)^2
=c_\lambda F_0'(T_0)
=F_\lambda'(T_0-\lambda).
$$

It descends to an isomorphism over \(\mathbb Q_p\) exactly when the square class of \(c_\lambda\) is trivial.  Therefore the Smith data obtained from this translated integral model are data for that explicit quadratic twist unless such a descent, together with its effect on the integral lattice, is proved.  Both square-root choices and the translation are retained.  No original-algebra conclusion below uses the translated chart without this qualification.
...
More generally, for \(m\ge2\), in a multiplicity-\(m\) local block
...
The restriction \(m\ge2\) is necessary.  When \(m=1\), the relation is \(\sigma^2=u(0)\in k^\times\), so multiplication by \(\sigma\) is invertible and is not nilpotent; the Jordan list in (29) does not apply.

### The original exterior cofactor determines a local quadratic field

There is a further arithmetic identification. For an original exterior state, put \(z=pZ\) and retain the exact exterior cofactor
...
These are written derivations. The repository checker `verify_defining_prime.py` now independently replays 80 exact identities covering the coefficient map, representative Smith strata, both \(p=1201\) witnesses, local nilpotent actions, the chart twist, collision comparison, explicit ES family, and local character example; its machine-readable receipt is `results/verification.json`. The checker is a certificate for those displayed calculations, not a replacement for the quantified arguments. The growing-\(k\) RH projected-current phase and the independent proper-source target minimum are not assigned values from this fixed finite receiver.
