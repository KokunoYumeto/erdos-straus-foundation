...
\caption{The common-denominator implication and one complete raw trace source.
In the displayed $p=13$, $a=4$, $R=3$ example, all raw words satisfying
$R\mid a+U$ are retained. The integral points are precisely
$U=2,8,26,104$, as TR8 predicts. The two middle words $U=2,8$ share
$S=95$ because their product is $a^2=16$; the exterior words are singleton
fibres. The figure therefore exhibits the nonzero kernel generator
$e_2-e_8$ rather than identifying the two source words. All plotted values
are generated with exact rational arithmetic by
\texttt{figures/trace\_integrality\_fibres.py}.}
\label{fig:trace-integrality-fibres}
\end{figure}
