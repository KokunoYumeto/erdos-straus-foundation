...
$\sum_{abc\le L}1\le L(\sum_{a\le L}1/a)^2$. This is a finite classification
in the original integers, not an assertion that a successful word always exists.
...
If $3\mid t$, work modulo three. The case $h=0$ makes $D=0$ and contradicts
...
only upper range issue. Its shape is $h-R=-4t$. Formula (13) proves the missing
canonical word and the sharp finite repair. The target is reconstructed by
...
\]
Among units $A\pmod M$ with $A\equiv3\pmod4$ and $(t/A)=1$, the implication
...
Since $(t/R)=1$, choose a nonzero residue $r_0$ satisfying
\[
 16tr_0^2\equiv1\pmod R.
\]
For example invert $4$ times either square root of $t$. Solve
\[
 r\equiv r_0\pmod R,\qquad
 r\equiv\frac{R+1}{4}h^{-1}\pmod{210}.
\]
