...
For M, all three roots and all derivative values are units or zero with distinct
reductions: the roots are $0,r/H,s/H$. Every geometric inverse in (5) is integral
over an unramified extension. Its geometric special fibre still has seven
points; precisely three are rational over $\mathbb F_p$. Thus counting only
rational special points also fails to distinguish the two channels.
...
Both entire shape lines $\alpha=-4$ and $\beta=-4$ are covered, without a bound
on their other coordinate.
\end{theorem}
\begin{proof}
Equation (15) makes $h$ odd and coprime to $c$. At every odd prime dividing $h$,
either $p\equiv-4c^2$ or $p^{-1}\equiv-4c^2$. Reciprocity gives
$(h/p)=(-1/h)$. Lemma~\ref{lem:character} makes this $-1$, so $h\equiv3\pmod4$.
...
For an explicit square-grade formula write $j=cm$, $d=\gcd(c,m)$.
In the $\alpha$ case,
$(h_M,r_M,\lambda_M)=(d^2,c/d,m/d)$; in the $\beta$ case it is
$(d^2,m/d,c/d)$. This also proves that the original shared budget has not been
used twice.
\end{proof}
...
\]
with the original coprimality and first-half inequalities. Conversely
$4rs-s^2=N$ follows directly from the shape, so this is the full fibre.
...
Within each marked branch, $c$ is recovered. Across an $\alpha$ branch with
$c_1$ and a $\beta$ branch with $c_2$, the selected target divisor is the same
exactly when $c_1c_2=j$. Different original sources in any one fibre remain
...
$(a,R,u,h,r,s,\kappa)=(806,175,20956,31,26,1,453)$ has
$\alpha=-144$, hence $c=6,j=8$. The selected $U=36$ is unavailable in $j^2=64$.
