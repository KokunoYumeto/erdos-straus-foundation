...
## SZ-20260920-063 — canonical/constructed metric separation and heat-scale split

Fix the complete divisor and source measure used by the pinned native
compression theorem. For the stipulated quartet
\[
\rho=\tfrac12\pm\delta\pm i\gamma,\qquad
0<\delta<\tfrac12,\quad \gamma>2,
\]
with common multiplicity \(m\), put
\[
D=k(m-1),\qquad q=(D+1)(k+1)^2,
\]
and retain the physical pullback metric
\(\widetilde G_{k,\eta}=\tau_\eta^kG_{k,\eta}\).
For every canonical cutoff sequence covered by NG20--NG21,
\[
\liminf_{k\to\infty}
\frac{\mathfrak d(\widetilde G_{k,\eta},G_{k,N})}
{q\log(q/k)}\ge2,
\qquad
\frac{\log\kappa(\widetilde G_{k,\eta},G_{k,N})}
{\log(q/k)}\longrightarrow+\infty .
\]
At the \(k\)-scale the canonical positive heat trace tends to zero, while
the constructed positive heat trace tends to
\[
\frac{\pi}{4t\delta\gamma}
\operatorname{erf}(\delta\sqrt t)\operatorname{erf}(\gamma\sqrt t),
\]
with the explicit finite error in MR12. The canonical heat-time and
packet-degree limits therefore do not commute.

- **Proof:** `research/incoming/es-rh-family-integration-c-20260920/proofs/01_METRIC_RETURN.md`, MR1--MR14.
- **Dependency:** NG20--NG21 at the exact source version in the module's `SOURCE_LEDGER.md`.
- **Certificate:** `checks/check_metric.py`, included in the ordinary/optimized replay.
- **Nonclaim:** the quartet is a stipulated counterfactual input. This does not assert an off-line zeta zero or a conclusion about the Riemann hypothesis.

## SZ-20260920-064 — exact joint-source allocation and certified range inverse

For the original scalar source \(X\), supplied section \(\mathcal R\), and
\[
H_Z=Z^*Z,\qquad F=I-JH_X^{-1}C_X,\qquad
T=G^{1/2}FH_Z^+F^*G^{1/2},
\]
the exact joint minimum is
\[
G_J=G^{1/2}(I+T)^{-1}G^{1/2}.
\]
For an onto observation \(\Lambda\), set \(K=\ker\Lambda\), write \(T\) in
blocks adapted to \(K\oplus K^\perp\), and put
\[
S_K=T_{11}-T_{12}(I+T_{22})^{-1}T_{12}^*.
\]
Then
\[
\log\det(I+T)
=\log\det(I+T_{22})+\log\det(I+S_K).
\]
If \(\mu=\lambda_{\min}^+(H_Z)\), every certified
\(0<\underline\lambda\le\mu\) gives
\[
\frac{\underline\lambda}{\underline\lambda+\varepsilon}C
\preceq C_\varepsilon\preceq C,
\quad
0\le\log\det G_{J,\varepsilon}-\log\det G_J
\le r_C\log\left(1+\frac{\varepsilon}{\underline\lambda}\right).
\]
For certified rank \(r_Z\),
\[
\mu\ge
\frac{\operatorname{pdet}H_Z}
{(\operatorname{tr}H_Z)^{r_Z-1}},
\]
so the right side supplies one computable choice of
\(\underline\lambda\).

- **Proof:** `research/incoming/es-rh-family-integration-c-20260920/proofs/02_JOINT_SOURCE.md`, JS1--JS15.
- **Certificate:** `checks/check_joint_minimum.py`, including the exact \(H_Z=[2]\) quantifier regression control.
- **Nonclaim:** physical-normal rank alone does not determine the rank of \(F\), the determinant gain, or the native cross Gram.

## SZ-20260920-065 — ES-labelled common reducing space and mixed covariance

For each chosen label matching, the Hermite translations
\[
t-t_{i,\rho}\longmapsto s-\rho
\]
give an exact isomorphism of the labelled local quotient systems, including
all retained nilpotent levels. For
\[
A_R=\kappa I+R,\qquad A_E=\theta I+R,
\]
the smallest common reducing space containing the cyclic image is
\[
\mathscr W^{\rm joint}
=\bigoplus_{\kappa,\theta,D:\,W_{\kappa,\theta,D}>0}
\operatorname{span}\{R^j1_{\kappa,\theta,D}:0\le j\le D\}.
\]
On this generated top-chain sector, the Casimir interpolation separates
the lengths. The adjoint ES leakage Gram is tridiagonal, and its neighboring
entry is the exact centre--length covariance
\[
(Z_E^*G_{\rm tensor}Z_E)_{j,j+1}
=g_j\eta^2(j+1)c_j.
\]
For a common quartet of multiplicity \(m\), labelled by an existing sorted
positive integral ES witness, and \(k\ge2\),
\[
\dim\mathscr W^{\rm joint}=(D+1)\binom{k+3}{3},
\quad
\operatorname{rank}L_E=(D+1)(k-1)^2,
\quad
\dim\ker L_E=4k(D+1).
\]
The \(k=1\) boundary has zero leakage and kernel dimension \(4(D+1)\).

- **Proof:** `research/incoming/es-rh-family-integration-c-20260920/proofs/03_ES_JETS.md`, ER1--ER15.
- **Certificate:** `checks/check_family.py`, including the ambient-Casimir counterexample and the \(k=1\) boundary.
- **Nonclaim:** this begins with an existing witness. It neither identifies ES denominators with zeta zeros nor constructs a witness at an unoccupied prime.

## SZ-20260920-066 — complete two-scale collision exponent diagram

Let \(e\ge f\ge1\), \(n=e+f\), and let the two divided-jet blocks carry
\(\eta=|z|^a\), \(a\ge0\), with the retained fixed metric hypotheses. If
\(\sigma_j\asymp|z|^{\nu_j}\), then for \(0\le a\le1\) the ordered exponents are
\[
0,a,\ldots,(e-1)a;
\qquad e-f+2j-1+a(f-j),\quad 1\le j\le f,
\]
whereas for \(a\ge1\) they are
\[
(a+1)j,\ (a+1)j+1\quad(0\le j<f);
\qquad aj+f\quad(f\le j<e).
\]
At \(a=1\), both lists equal \(0,1,\ldots,n-1\). Their determinant order is
\[
ef+\frac a2\{e(e-1)+f(f-1)\},
\]
and every inverse-exterior exponent is
\[
\mathfrak e_r(a)=\sum_{j=n-r+1}^{n}\nu_j(a).
\]
CP11 gives the balanced-ray constants, with row weights
\(\sqrt{c_\alpha}\) and rank-\(r\) coordinate changes controlled by
\(\|\wedge^rC^{\pm1}\|\).

- **Proof:** `research/incoming/es-rh-family-integration-c-20260920/proofs/04_COLLISION_PHASE.md`, CP1--CP11.
- **Certificate:** `checks/check_collision.py`, included in the ordinary/optimized replay.
- **Nonclaim:** a degenerating physical multiplier or metric must be restored explicitly; it cannot be discarded from the exponent calculation.
